	...
	do
	 {
	  ...
	  for (i=0;i<fN; i++)
	   {
		...
		dif = abs(fV[i] -fR[0]);		 // Calculamos la distancia respecto el elemento actual y el centroide (un primer centroide, inicializamos basicamente el dif_menor)		|S2|
		for (j=1;j<fK;j++)				 // Miramos los otros clusters																												|S3|
		  if (abs(fV[i] -fR[j]) < dif)   // SI la fistancia entre este elemento y el siguiente centroide (cluster), es menor que el anterior dif_min:								|S4|
		   {																																							//			|S5|
			min = j;					// El siguiente cluster pasa a ser el cluster_minimo actual del elemento actual (de momento)												|S6|
			dif = abs(fV[i] -fR[j]);	// La distancia minima es la nueva encontrada																								|S7|
		   }	
		fD[i] = min;					// Asignamos finalmente el elemento actual el cluster con el que guarde menos distancia con su centroide									|S8|	
	   }
	  ...
	  for(i=0;i<fN;i++)
	   {
		fS[fD[i]] += fV[i];		// fF[indice_cluster_al_q_pertenece_este_elemento] = valor_numerico_de_este_elemento						|S11|
		fA[fD[i]] ++;			// fA[indice_cluster_al_q_pertenece_este_elemento]++, indicamos que tiene 1 elemento más ese cluster		|S12|
	   }
	  ...
	  for(i=0;i<fK;i++)						// miramos todos los clusters (num d clusters = fK)
	   {
		t = fR[i];							// pillamos el centroide actual y lo guardamos en 't'																															|S14|
		if (fA[i]) fR[i] = fS[i]/fA[i];     // si el cluster tiene algun miembro, calculamos entonces el nuevo centroide --> media d todos los valores numericos d todos los elementos que forman parte del cluster			|S15|
		dif += abs(t - fR[i]);				// acumulamos diferencia absoluta entre el centroide anterior ('t') y el nuevo																									|S16|
	   }
	   iter ++;								// num d iteraciones totales (cada iteración implica asignar todos los elementos, y recalcular los centroides)																	|S17|	
	 } while(dif);							// Hacemos esto siempre y cuando se mejoren los centroides, cuando se estabilicen llegamos al gin (cuando los centroides no hayan cambiado entre iteraciones, ese es el final)  |S18|
  ...
  while (i <= f)
   {
    if (vtmp < pi) {			// si valor temporal menor q el pivote, se coloca a la izquierda del pivote			|S1|
	  fV[i-1] = vtmp;			// colocamos el valor en la posición anterior    									|S2|
	  fA[i-1] = vta;			// movemos tmb el valor asociado													|S3|
	  i ++;						// pasamos al siguiente elemento													|S4|
